...
#   Created: 8/18/2020
#   Description: Users Table Python/SQL
...
with db_connect("conference.sqlite") as db:     # connect to database
    ...
    create_users_table = ''' CREATE TABLE IF NOT EXISTS users (
                                ...
                                lastname text,
                                password text);'''
    users_template = '''INSERT INTO users (
                                username,firstname,lastname,password) VALUES (?, ?, ?, ?);'''

    curs = assign_cursor(db)                                    # assign cursor
    users_file = open_file('users.csv')                         # open data file
    recreate_table(db, delete_users_table, create_users_table)  # clear old table and recreate
    load_file_line_to_db(users_file, users_template, db, curs)  # load table from csv file

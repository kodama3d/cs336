...
# Created: 7/28/2020
# Description: Python 8 tags page for assignment
...
line_dict = {       # dictionary for html container line info
    ...
    'pgcol': {'string': 'class="nt8_col"', 'ln': 0, 'sp': 0, 'head': '', 'tail': '</div>\n'},
    'row': {'string': 'class="nt8_row"', 'ln': 0, 'sp': 0, 'head': '', 'tail': '</div>\n'},
    'nt': {'string': 'class="nt8"', 'ln': 0, 'sp': 0, 'head': '', 'tail': '</div>\n'},
...
# loads registration data from open file into dictionary
...
html_file = open_file('../templates/nametags8_template.html')
html_out = del_file('../templates/nametags8.html')
load_registration(reg_sql)
load_html(html_file, line_dict, line_total)

tot_tags = len(reg_data)            # total tags
tot_pg = math.ceil(tot_tags / 8)    # total pages to print
...
for pg in range(0, tot_pg):             # loop creating enough page templates for all tags
    ...
    for row in range(0, 4):         # 4 rows on page
